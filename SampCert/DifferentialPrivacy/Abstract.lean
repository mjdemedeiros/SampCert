...
-/
theorem privPostProcess_NonZeroNQ {nq : Mechanism T U} {f : U → V}
(nn : NonZeroNQ nq) (sur : Function.Surjective f) :
...
-/
lemma privPostProcess_ext [dps : DPSystem T] : {U : Type} → [MeasurableSpace U] → [Countable U] → [DiscreteMeasurableSpace U] → [Inhabited U] → { pp : U → V } → ∀ m : Mechanism T U, ∀ ε₁ ε₂ : ℕ+, ∀ ε : ℝ,
    dps.postprocess_prop_f pp ->dps.prop m (ε₁ / ε₂) -> (ε = ε₁ / ε₂) → dps.prop (privPostProcess m pp) ε := by
