...
def readCapsidV_while (e : Expr) : MetaM Name :=
    match e with
    | Expr.app (Expr.app _ (Expr.lam _ _ (Expr.app (Expr.const capsidWhileIdent _) _) _)) _ =>
          pure capsidWhileIdent
    | _ => throwError "panic: cannot read capsid while ident"

def readCapsidV_until (e : Expr) : MetaM Name :=
    match e with
    | Expr.app (Expr.app _ _ ) (Expr.lam _ _ (Expr.app (Expr.const capsidWhileIdent _) _) _) =>
          pure capsidWhileIdent
    | _ => throwError "panic: cannot read capsid until ident"
...
variable (capsidM : Name)
variable (capsidV : Expr)
...
partial def toDafnyExpr (dname : String) (env : List String) (e : Expr) : MetaM Expression := do
  ...
  | .app .. => (e.withApp fun fn args => do
      ...
      | ``bind => do
        ...
        let v1 ← toDafnyExpr dname env args[4]!
        -- IO.println s!" --> v1 complete"
        let v2 ← toDafnyExpr dname env args[5]!
      ...
      | _ =>
        -- If only there were some way to use match guard expressions in lean (there probably is)
        let while_ident <- (readCapsidV_while capsidV)
        if (name = while_ident)
          then return .prob_while (← toDafnyExpr dname env args[1]!) (← toDafnyExpr dname env args[2]!) (← toDafnyExpr dname env args[3]!)

        let until_ident <- (readCapsidV_until capsidV)
        if (name = until_ident)
          then return .prob_until (← toDafnyExpr dname env args[1]!) (← toDafnyExpr dname env args[2]!)
...
-- Is the mutual is ending the scope of the section variables? odd
variable (capsidM : Name)
variable (capsidV : Expr)
...
def toDafnyTypTop (env : List String) (e: Expr) : MetaM ((List Typ) × Typ) := do
  ...
  | (.app (.const t ..) arg) =>
          if t = capsidM
            then return ([],← toDafnyTyp capsidM capsidV [] arg)
  ...
  | .forallE binder domain range _ =>
    ...
    return ((← toDafnyTyp capsidM capsidV env domain) :: r.1 , r.2)
...
partial def toDafnyExprTop (dname : String) (num_args : Nat) (names : List String) (e : Expr) : MetaM (List String × Expression) := do
  ...
  | .app .. =>
    e.withApp fun fn args =>
      match fn with
      | (.const ``WellFounded.fix ..) => toDafnyExprTop dname num_args names (args[4]!)
      -- Very wrong
      -- | (.const ``pure ..) => do
      --   let to_translate := args[3]!
      --   let de <- toDafnyExpr dname names to_translate
  ...
  | .lam binderName _ body _ =>
    ...
    then
      match body with
      | (.lam _ _ body _) => return (sig_names, ← toDafnyExpr capsidM capsidV dname ("dummy" :: sig_names.reverse) (chopLambda body))
      | _ => return (sig_names, ← toDafnyExpr capsidM capsidV dname sig_names.reverse body)
    ...
    | ConstantInfo.defnInfo _ =>
      ...
      | some (capsidM, progT) => do
        ...
        | some (capsid_instance_name, program_name) => do
          ...
          let (inParamTyp, outParamTyp) ← toDafnyTypTop capsidM capsidInfo.value [] progT
          -- IO.println s!" - Got capsid value {capsidInfo.value}"

          let (inParam, body) ← toDafnyExprTop capsidM (capsidInfo.value) declName.toString (List.length inParamTyp) [] (progInfo.value)
